...
const listSessionFiles = async (sessionId) => {
    const sessionDir = path.join(config.storage.localDir, sessionId);
    try {
        const entries = await fs.readdir(sessionDir);
        return entries.map((file) => path.join(sessionDir, file));
    }
    catch (error) {
...
const getSessionContent = async (sessionId) => {
    try {
        ...
        if (source !== 'capture') {
            ...
            logger.info({ sessionId, source, contentLength: content.length }, 'Using manual input content');
            return content;
        }
...
export const processSessionPipeline = async (sessionId) => {
