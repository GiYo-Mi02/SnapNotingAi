import { createManualSession } from '../services/manualInputService.js';
import { extractTextFromDocument, validateDocumentFile } from '../services/documentService.js';
 ...
 * Body: { content: string }
 */
export const submitManualText = async (req, res, next) => {
    try {
        ...
        processSessionPipeline(session.id).catch((pipelineErr) => {
            logger.error({ err: pipelineErr, sessionId: session.id }, 'Background pipeline failed');
        });
        logger.info({
 ...
 */
export const submitAudioTranscript = async (req, res, next) => {
    try {
