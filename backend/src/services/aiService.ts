...
import { type QuizQuestion } from '../types/session.js'

const buildClient = (): OpenAI | null => {
...
export const generateSummary = async (textSegments: string[]): Promise<string> => {
...
Make questions diverse (conceptual, factual, application-based). Each option should be plausible but only one correct.`
  ...
  try {
    const parsed = JSON.parse(jsonText) as QuizSchemaResponse
    ...
    if (jsonMatch !== null) {
      try {
        const parsed = JSON.parse(jsonMatch[0]) as QuizSchemaResponse
        return parsed.questions ?? []
      } catch {
        return []
      }
